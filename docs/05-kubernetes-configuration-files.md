...
for i in 0 1 2; do
  ...
  --query 'Reservations[].Instances[].PublicIpAddress'| jq .[0] | sed 's/"//g')
  scp -i $KEY_PATH -o "StrictHostKeyChecking no" admin.kubeconfig \
  kube-controller-manager.kubeconfig kube-scheduler.kubeconfig ubuntu@$IP:~/
done
```
